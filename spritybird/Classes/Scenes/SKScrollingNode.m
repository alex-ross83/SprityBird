...
- (void) update:(NSTimeInterval)currentTime
{
    [self.children enumerateObjectsUsingBlock:^(SKNode * _Nonnull child, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        CGRect layerSize = [child calculateAccumulatedFrame];
        if (child.position.x <= -layerSize.size.width) {
            float delta = child.position.x+layerSize.size.width;
            child.position = CGPointMake(layerSize.size.width*(self.children.count-1)+delta, child.position.y);
        }
    }];
}
